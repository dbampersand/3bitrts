...
function update(dt)
    ...
    if (spawnedBoss == false and timer > 60) then
        CreateObject("assets/encounters/02/boss.lua",120,22,TYPE_ENEMY);
        spawnedBoss = true
    end
    ...
    if (spawnedBoss == false) then
        ...
        if (secondWave == false and timer > 30) then
            local enemyObjs = GetAllObjsByFriendliness(TYPE_ENEMY);
            local numObjs = #enemyObjs;
            local ob = CreateObject("assets/enemies/berzerker/berzerker.lua",120,22,TYPE_ENEMY);
            SetObjAggroRadius(ob,999)
        ...
        if (thirdWave == false and timer > 45) then
            local enemyObjs = GetAllObjsByFriendliness(TYPE_ENEMY);
            local numObjs = #enemyObjs;
            local ob = CreateObject("assets/enemies/shield_warrior/shield_warrior.lua",120,22,TYPE_ENEMY);
            SetObjAggroRadius(ob,999)
